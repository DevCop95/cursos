...
export const COURSE_ICONS = { 'pentesting-101': 'radar', 'git-github-101': 'account_tree' };
export const courseIcon = id => COURSE_ICONS[id] || 'school';

/**
 * Datos para pintar el rango a partir de lo que devuelve user_stats:
 * { name, icon, tone, points, next, remaining, pct } (pct: avance hacia el siguiente rango, 100 si es el último).
 */
export function rankView(stats) {
  ...
  const pct = to === null ? 100 : Math.max(0, Math.min(100, Math.round(((points - from) / Math.max(1, to - from)) * 100)));
  return {
    name: def.name, icon: def.icon, tone: def.tone, points,
    next: typeof stats.next_rank === 'string' ? stats.next_rank : null,
    remaining: to === null ? 0 : Math.max(0, to - points),
    pct
  };
}
