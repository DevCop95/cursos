 ...
 */
import { esc, toCsv } from '../lib/html.js?v=dev101x-v56';
import { isCloudEnabled } from '../config.js?v=dev101x-v56';
import { COURSE, LAB_STEPS } from '../content.js?v=dev101x-v56';
import { computeProgress, isLabDone, TOTAL_LESSONS } from '../lab.js?v=dev101x-v56';
import { adminListStudents, fetchCourses, adminSetCourseOverride, adminSetAccessLevel, adminUpdateCourse, adminResetProgress, fetchAccessRequests, adminRejectAccessRequest, fetchMessages, adminRevokeCourse, fetchUserStats } from '../cloud.js?v=dev101x-v56';
import { rankView } from '../lib/ranks.js?v=dev101x-v56';
import { openAdminThread } from './messages.js?v=dev101x-v56';
import { avatarFor, showToast, openDialog } from '../ui.js?v=dev101x-v56';
import { activityStatus, filterByActivity, lastActivity, relativeTime } from '../lib/activity.js?v=dev101x-v56';
import { courseAccess, ACCESS_LEVELS } from '../lib/access.js?v=dev101x-v56';
...
export function openUserDetails(userId) {
  ...
  fetchUserStats(userId).then(stats => {
    ...
    el.innerHTML = `<span class="inline-flex items-center gap-1 px-1.5 py-px rounded-md bg-term border ${v.tone} font-bold"><span class="material-symbols-outlined text-[13px]" aria-hidden="true">${esc(v.icon)}</span>${esc(v.name)}</span> <span class="ml-1">${v.ownership}% · ${v.points} pts</span>`;
  }).catch(() => {});
}
